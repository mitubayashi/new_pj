...
<?php
    ...
    //ファイルチェック
    $error_data = keihi_fileinsert_check($_SESSION['fileinsert']);
    $error_list .= "<div class='list_scroll' style='margin-top: 10px; margin-bottom: 10px;'>";
    $error_list .= "<table>";
    $error_list .= "<tr><th>プロジェクトコード</th><th>社員番号</th><th>旅費交通費</th><th>その他</th><th>日付</th><th>判定</th></tr>";
    for($i = 0; $i < count($error_data); $i++)
    {
        $error_list .= "<tr>";
        $error_list .= "<td>".$error_data[$i]['PJCODE']."</td>";
        $error_list .= "<td>".$error_data[$i]['STAFFID']."</td>";
        $error_list .= "<td>".$error_data[$i]['KOUTUHI']."</td>";
        $error_list .= "<td>".$error_data[$i]['SONOTA']."</td>";
        $error_list .= "<td>".$error_data[$i]['HIDUKE']."</td>";
    ...
    <head>
        ...
        <script>
            ...
            if(judge)
            {
                ...
                {
                    location.href = "./keihifileinsertComp.php";
                }
            }
        </script>
    </head>
        ...
        <div class="body_area">
            <form action="listJump.php" method="post" enctype="multipart/form-data">
                <?php
                    ...
                    echo "<input type ='submit' value = '取込画面に戻る' name = 'keihifileinsert_5_button' class = 'list_button' onClick ='isCancel = true;' style='width: 120px;'>";
                ?>
            </form>
        </div>
